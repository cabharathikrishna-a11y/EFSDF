...
class AppViewModel(application: Application, private val repository: LocalRepository) : AndroidViewModel(application) {
    ...
    private fun startRealtimeOwnUserSync() {
        ...
        try {
            val url = com.example.api.FirebaseConfig.getDatabaseUrl(getApplication())
            ...
            val listener = object : com.google.firebase.database.ValueEventListener {
                override fun onDataChange(snapshot: com.google.firebase.database.DataSnapshot) {
                    ...
                    val remoteUser = com.example.api.FirebaseSyncManager.parseUserSnapshot(snapshot) ?: return
                    
                    viewModelScope.launch {
                        try {
                            val localDeviceId = FocusTimerManager.getOrCreateDeviceId(getApplication())
                            val remoteDeviceId = remoteUser.lastUpdatedDeviceId ?: ""
                            val remoteTimestamp = remoteUser.lastUpdatedTimestamp ?: 0L

                            // 1. RECONCILE FOCUS STATE AND REMOTE BUTTON ACTIONS INSTANTLY (Timer sync)
                            reconcileRemoteFocusState(remoteUser)

                            // 2. RECONCILE DATA / APP STATE VIA GOOGLE DRIVE BACKUP (Conflict Resolution)
                            if (remoteDeviceId.isNotEmpty() && remoteDeviceId != localDeviceId && remoteTimestamp > 0L) {
                                val localLastSync = prefs.getLong("gd_all_last_sync_timestamp", 0L)
                                val localLastDbModified = prefs.getLong("local_db_last_modified_timestamp", 0L)
                                
                                if (remoteTimestamp > localLastSync && remoteTimestamp > localLastDbModified) {
                                    android.util.Log.i("AppViewModel", "Newer remote database changes detected from device $remoteDeviceId at timestamp $remoteTimestamp. Triggering automatic, silent Google Drive restore.")
                                    val (success, message) = com.example.util.GoogleDriveSyncManager.restoreAllAppData(getApplication(), appDatabase) { _ -> }
                                    if (success) {
                                        android.util.Log.i("AppViewModel", "Auto-restore success: $message")
                                        prefs.edit().putLong("gd_all_last_sync_timestamp", remoteTimestamp).apply()
                                        refreshLocalDbCaches()
                                    } else {
                                        android.util.Log.e("AppViewModel", "Auto-restore failed: $message")
                                    }
                                }
                            }
                        } catch (e: Exception) {
                            android.util.Log.e("AppViewModel", "Error in real-time own user sync onDataChange", e)
                        }
                    }
                }

                override fun onCancelled(error: com.google.firebase.database.DatabaseError) {
                    android.util.Log.w("AppViewModel", "Own user real-time listener cancelled: ${error.message}")
                }
            }

            ref.addValueEventListener(listener)
            ownUserListener = listener
            android.util.Log.i("AppViewModel", "Successfully registered real-time WebSocket listener on Firebase /users/$username for perfect multi-device sync.")
        } catch (e: Exception) {
    ...
    private fun stopRealtimeOwnUserSync() {
        try {
            val ref = ownUserRef
            val listener = ownUserListener
            if (ref != null && listener != null) {
                ref.removeEventListener(listener)
                android.util.Log.i("AppViewModel", "Removed real-time own user Firebase listener.")
            }
        } catch (e: Exception) {
